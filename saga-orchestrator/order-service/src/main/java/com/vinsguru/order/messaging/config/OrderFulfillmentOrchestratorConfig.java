...
public class OrderFulfillmentOrchestratorConfig {
    ...
    private String getDestination(Request request) {
        if (request instanceof PaymentRequest e) {
            return PAYMENT_REQUEST_CHANNEL;
        } else if (request instanceof InventoryRequest e) {
            return INVENTORY_REQUEST_CHANNEL;
        } else if (request instanceof ShippingRequest e) {
            return SHIPPING_REQUEST_CHANNEL;
        } else {
